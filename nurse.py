...
def searchForm():
    ...
    title = tk.Label(root, text='Search Nurse', bg='white', fg='#57a1f8',
                     font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600, y=10)
    pid_label = tk.Label(root, text="NurseID:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                         fg='#00a0da')
    ...
    btn1 = tk.Button(root, text='Search Nurse', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600, y=600)

    root.mainloop()

def search(details,root):
    query = f"SELECT * FROM NURSE WHERE NurseID = {details[0]} OR FirstName = '{details[1]}'"
...
def remove(pid,root):
    query = f"DELETE FROM NURSE WHERE NurseID = {int(pid)}"
    cursor.execute(query)
    connection.commit()
    success = tk.Label(root, text="Nurse Deleted Successfully", bg='white',
...
def removeForm():
    ...
    root = tk.Tk()
    root.config(bg='white')
    title = tk.Label(root, text='Delete Nurse', bg='white', fg='#57a1f8',
                     font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600, y=10)
    pid_label = tk.Label(root, text="NurseID:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                         fg='#00a0da')
    ...
    btn1 = tk.Button(root, text='Delete Nurse', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600, y=600)
...
def updateForm():
    ...
    root = tk.Tk()
    root.config(bg='white')
    title = tk.Label(root, text='Update Nurse', bg='white', fg='#57a1f8', font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600, y=10)
    pid_label = tk.Label(root, text="NurseID:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    pid_label.place(x=100, y=150)
    ...
    btn1 = tk.Button(root, text='Update Nurse', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600, y=600)

    root.mainloop()

def update(details,root):
    query = f"UPDATE NURSE SET {details[1]} = '{details[2]}' WHERE NurseID = {int(details[0])}"
    cursor.execute(query)
    connection.commit()
    success = tk.Label(root, text="Nurse Updated Successfully", bg='white',
                       font=('Microsoft YaHei UI Light', 24, 'bold'), fg='#00a0da')
    success.place(x=520, y=700)
...
def display_records(rows,second_frame,root):
    ...
    btn1 = tk.Label(buttonframe,text="NurseID",bg='#00a0da',fg='white',height=2,borderwidth=1, relief=tk.RAISED)
    ...
    btn7.grid(row=0, column=6, sticky="nsew")
...
def insertDetails(details,root):
    query = f"INSERT INTO NURSE(FirstName,MiddleName,LastName,Gender,Experience,Age) VALUES('{details[0]}','{details[1]}','{details[2]}','{details[3]}','{details[4]}',{details[5]})"
    cursor.execute(query)
    connection.commit()
    success = tk.Label(root, text="Nurse Added Successfully", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),fg='#00a0da')
    success.place(x=550,y=700)

def entryForm(root):
    details = []
    def getDetails():
        details = [first_name.get(),middle_name.get(),last_name.get(),gender.get(),dob.get(),bg.get()]
    ...
    dob_label = tk.Label(root, text="Experience:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    dob_label.place(x=800, y=150)
    dob = tk.Entry(root, font=('Microsoft YaHei UI Light', 22, 'bold'), borderwidth=2)
    dob.place(x=1015, y=150)

    bg_label = tk.Label(root, text="Age:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    bg_label.place(x=800, y=250)
    bg = tk.Entry(root, font=('Microsoft YaHei UI Light', 22, 'bold'), borderwidth=2)
    bg.place(x=1015, y=250)

    btn1 = tk.Button(root, text='Add Nurse', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600,y=600)

def AddNurse():
    root = tk.Tk()
    root.config(bg='white')
    title = tk.Label(root,text='Add Nurse',bg='white',fg='#57a1f8',font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600,y=10)
    entryForm(root)
    root.mainloop()
...
def NursePage():
    ...
    label = tk.Label(root, text='Nurse', fg='#00a0da', bg='white',
                     font=('Arial', 39, 'bold'))
    ...
    mainpage_pic2 = tk.PhotoImage(file=r'C:\Users\Tarun Parthiban\PycharmProjects\Hospital_Mgmt\nurse.png')
    tk.Label(root, image=mainpage_pic2, bg='white').place(x=265, y=210)

    buttonframe = tk.Frame(root, bg='white')
    buttonframe.columnconfigure(0, weight=1)

    btn1 = tk.Button(buttonframe, text='Add Nurse', font=('Arial', 18), width=20,height=2,command=AddNurse)
    ...
    btn4 = tk.Button(buttonframe, text='Remove Nurse', font=('Arial', 18), width=20,height=2,command=removeForm)
    btn4.grid(row=0, column=1, pady=15, padx=15)

    btn5 = tk.Button(buttonframe, text='Search Nurse', font=('Arial', 18), width=20,height=2,command=searchForm)

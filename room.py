...
def searchForm():
    ...
    pid_label = tk.Label(root, text="Room No:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                         fg='#00a0da')
...
def search(details,root):
    query = f"SELECT * FROM room WHERE roomno = {details[0]} OR pid = {details[1]}"
...
def remove(pid,root):
    query = f"DELETE FROM room WHERE roomno = {int(pid)}"
    cursor.execute(query)
    connection.commit()
    success = tk.Label(root, text="Checked Out Successfully", bg='white',
...
def removeForm():
    ...
    root = tk.Tk()
    root.config(bg='white')
    title = tk.Label(root, text='Check Out', bg='white', fg='#57a1f8',
                     font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600, y=10)
    pid_label = tk.Label(root, text="RoomNo:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                         fg='#00a0da')
    pid_label.place(x=100, y=150)
    pid = tk.Entry(root, font=('Microsoft YaHei UI Light', 22, 'bold'), borderwidth=2)
    pid.place(x=315, y=150)

    btn1 = tk.Button(root, text='Check Out', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600, y=600)
...
def display_records(rows,second_frame,root):
    ...
    btn1 = tk.Label(buttonframe,text="RoomNo",bg='#00a0da',fg='white',height=2,borderwidth=1, relief=tk.RAISED)
    ...
    btn3 = tk.Label(buttonframe, text="FromDate",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
    btn3.grid(row=0, column=2, sticky="nsew")
    btn9 = tk.Label(buttonframe, text="ToDate",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
    btn9.grid(row=0, column=3, sticky="nsew")

    btn4 = tk.Label(buttonframe, text="Room Type",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
    btn4.grid(row=0, column=4, sticky="nsew")
    btn5 = tk.Label(buttonframe, text="NurseID",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
...
def insertDetails(details,root):
    query = f"INSERT INTO room VALUES({details[0]},'{details[1]}','{details[2]}','{details[3]}','{details[4]}',{details[5]})"
    cursor.execute(query)
    connection.commit()
    success = tk.Label(root, text="Checked In Successfully", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),fg='#00a0da')
    success.place(x=550,y=700)

def entryForm(root):
    details = []
    def getDetails():
        details = [first_name.get(),middle_name.get(),last_name.get(),gender.get(),dob.get(),nurse.get()]
        insertDetails(details,root)

    first_name_label = tk.Label(root, text="Room No", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),fg='#00a0da')
    ...
    last_name_label = tk.Label(root, text="From Date:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    ...
    gender_label = tk.Label(root, text="ToDate:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    ...
    dob_label = tk.Label(root, text="Room Type:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    dob_label.place(x=800, y=150)
    dob = tk.Entry(root, font=('Microsoft YaHei UI Light', 22, 'bold'), borderwidth=2)
    dob.place(x=1015, y=150)

    nurse_label = tk.Label(root, text="Nurse ID:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                        fg='#00a0da')
    nurse_label.place(x=800, y=250)
    nurse = tk.Entry(root, font=('Microsoft YaHei UI Light', 22, 'bold'), borderwidth=2)
    nurse.place(x=1015, y=250)

    btn1 = tk.Button(root, text='Check In', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600,y=600)

def AddRoom():
    root = tk.Tk()
    root.config(bg='white')
    title = tk.Label(root,text='Check In',bg='white',fg='#57a1f8',font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600,y=10)
    entryForm(root)
    root.mainloop()
...
def RoomPage():
    ...
    label = tk.Label(root, text='Room', fg='#00a0da', bg='white',
                     font=('Arial', 39, 'bold'))
    ...
    mainpage_pic2 = tk.PhotoImage(file=r'room.png')
    tk.Label(root, image=mainpage_pic2, bg='white').place(x=185, y=290)

    buttonframe = tk.Frame(root, bg='white')
    buttonframe.columnconfigure(0, weight=1)

    btn1 = tk.Button(buttonframe, text='Check In', font=('Arial', 18), width=20,height=2,command=AddRoom)
    ...
    btn4 = tk.Button(buttonframe, text='Check Out', font=('Arial', 18), width=20,height=2,command=removeForm)
    btn4.grid(row=3, column=0, pady=15, padx=15)

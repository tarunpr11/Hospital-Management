...
def searchForm():
    ...
    title = tk.Label(root, text='Search Lab Records', bg='white', fg='#57a1f8',
                     font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600, y=10)
    pid_label = tk.Label(root, text="TestID:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                         fg='#00a0da')
    ...
    btn1 = tk.Button(root, text='Search Test Results', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600, y=600)

    root.mainloop()

def search(details,root):
    query = f"SELECT * FROM LAB WHERE TestID = {details[0]} OR PID = {details[1]}"
...
def display_records(rows,second_frame,root):
    ...
    btn1 = tk.Label(buttonframe,text="TestID",bg='#00a0da',fg='white',height=2,borderwidth=1, relief=tk.RAISED)
    ...
    btn3 = tk.Label(buttonframe, text="TestType",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
    btn3.grid(row=0, column=2, sticky="nsew")
    btn9 = tk.Label(buttonframe, text="Results",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
    btn9.grid(row=0, column=3, sticky="nsew")

    btn4 = tk.Label(buttonframe, text="DocID",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
    btn4.grid(row=0, column=4, sticky="nsew")
    btn5 = tk.Label(buttonframe, text="TestDate",bg='#00a0da',fg='white',borderwidth=1, relief=tk.RAISED)
...
def insertDetails(details,root):
    query = f"INSERT INTO LAB(PID,TestType,Results,DocID,TestDate) VALUES({details[0]},'{details[1]}','{details[2]}','{details[3]}','{details[4]}')"
    cursor.execute(query)
    connection.commit()
    success = tk.Label(root, text="Lab Record Added Successfully", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),fg='#00a0da')
    success.place(x=480,y=700)
...
def entryForm(root):
    ...
    middle_name_label = tk.Label(root, text="Test Type:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    ...
    last_name_label = tk.Label(root, text="Results:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    ...
    gender_label = tk.Label(root, text="DocID:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    ...
    dob_label = tk.Label(root, text="Test Date:", bg='white', font=('Microsoft YaHei UI Light', 24, 'bold'),
                                fg='#00a0da')
    dob_label.place(x=800, y=150)
    dob = tk.Entry(root, font=('Microsoft YaHei UI Light', 22, 'bold'), borderwidth=2)
    dob.place(x=1015, y=150)

    btn1 = tk.Button(root, text='Add Lab Result', font=('Arial', 18), width=20, height=2, command=getDetails)
    btn1.place(x=600,y=600)

def AddLab():
    root = tk.Tk()
    root.config(bg='white')
    title = tk.Label(root,text='Add Lab Result',bg='white',fg='#57a1f8',font=('Microsoft YaHei UI Light', 49, 'bold'))
    title.place(x=600,y=10)
    entryForm(root)
    root.mainloop()
...
def LabPage():
    ...
    label = tk.Label(root, text='Lab', fg='#00a0da', bg='white',
                     font=('Arial', 39, 'bold'))
    ...
    mainpage_pic2 = tk.PhotoImage(file=r'lab.png')
    tk.Label(root, image=mainpage_pic2, bg='white').place(x=0, y=210)

    buttonframe = tk.Frame(root, bg='white')
    buttonframe.columnconfigure(0, weight=1)

    btn1 = tk.Button(buttonframe, text='Add Lab Result', font=('Arial', 18), width=20,height=2,command=AddLab)
    ...
    btn5 = tk.Button(buttonframe, text='Search Lab Result', font=('Arial', 18), width=20,height=2,command=searchForm)
